...
/**
 * MARK 管理quorum协议的实现. This class manages the quorum protocol. 
 * 
 * There are three states this server can be in:
 * <ol>
 * <li>Leader election - each server will elect a leader (proposing itself as a leader
 * initially).</li>
 * <li>Follower - the server will synchronize with the leader and replicate any transactions.</li>
 * <li>Leader - the server will process requests and forward them to followers. A majority of
 * followers must log the request before it can be accepted.
 * </ol>
 * MARK 当前leader的视图: xid, myid, leader_id, leader_zxid 
 * 
...
public class QuorumPeer extends Thread implements QuorumStats.Provider {
  ...
  /**
   * MARK ZK数据库, 用于后续初始化的server中. ZKDatabase is a top level member of quorumpeer which will be used
   * in all the zookeeperservers instantiated later. Also, it is created once on bootup and only
   * thrown away in case of a truncate message from the leader
   */
  ...
  /*
   * To enable observers to have no identifier, we need a generic identifier at least for
   * QuorumCnxManager. We use the following constant to as the value of such a generic identifier.
   */
  ...
  /** MARK the epoch number of the last NEWEPOCH packet accepted. */
  private long acceptedEpoch = -1;
  /** MARK the epoch number of the last NEWLEADER packet accepted. */
  ...
  public QuorumPeer(Map<Long, QuorumServer> quorumPeers, File dataDir, File dataLogDir,
      int electionType, long myid, int tickTime, int initLimit, int syncLimit,
      ServerCnxnFactory cnxnFactory) throws IOException {
    this(quorumPeers, dataDir, dataLogDir, electionType, myid, tickTime, initLimit, syncLimit,
        false, cnxnFactory, new QuorumMaj(countParticipants(quorumPeers)));
  }

  public QuorumPeer(Map<Long, QuorumServer> quorumPeers, File dataDir, File dataLogDir,
      int electionType, long myid, int tickTime, int initLimit, int syncLimit,
      boolean quorumListenOnAllIPs, ServerCnxnFactory cnxnFactory, QuorumVerifier quorumConfig)
      throws IOException {
    this();
    ...
    if (quorumConfig == null) this.quorumConfig = new QuorumMaj(countParticipants(quorumPeers));
  ...
  public QuorumPeer(Map<Long, QuorumServer> quorumPeers, File snapDir, File logDir, int clientPort,
    ...
    this(quorumPeers, snapDir, logDir, electionAlg, myid, tickTime, initLimit, syncLimit, false,
        ServerCnxnFactory.createFactory(new InetSocketAddress(clientPort), -1), quorumConfig);
  }

  /** MARK quorum服务器信息. */
  public static class QuorumServer {
      public InetSocketAddress addr;                          // MARK follower连接地址
      public InetSocketAddress electionAddr;                  // MARK leader选举地址
      public long id;
      public LearnerType type = LearnerType.PARTICIPANT;      // MARK 默认为参与者
  ...
  /**
   * MARK learner类型: 参与者, 观察者 A peer can either be participating, which implies that it is willing
   * to both vote in instances of consensus and to elect or become a Leader, or it may be observing
   * in which case it isn't. We need this distinction to decide which ServerState to move to when
   ...
   *             the udp-based versions of leader election, which we are also deprecating. This
   *             class simply responds to requests for the current leader of this node.
   *             <p>
   *             The request contains just an xid generated by the requestor.
   *             <p>
   *             The response has the xid, the id of this server, the id of the leader, and the zxid
   *             of the leader.
   */
  @Deprecated
  ...
  public synchronized void start() {
    loadDataBase();           // MARK 加载数据库
    cnxnFactory.start();      // MARK 启动server连接工厂
    startLeaderElection();    // MARK 准备leader选举: 算法

    super.start();            // MARK 执行线程启动: run()
  }
  ...
  private void loadDataBase() {
    ...
    try {
      // MARK 从日志文件和快照文件, 重建内存数据库
      zkDb.loadDataBase();

      // load the epochs
      final long lastProcessedZxid = zkDb.getDataTree().lastProcessedZxid;
      ...
      try {
        ...
        if (epochOfZxid > currentEpoch && updating.exists()) {
          ...
          setCurrentEpoch(epochOfZxid);               // MARK updatingEpoch存在, 且最后已处理事务中epoch比当前epoch大时, 更新当前epoch
          if (!updating.delete()) {
        ...
        writeLongToFile(CURRENT_EPOCH_FILENAME, currentEpoch); // MARK currentEpoch文件不存在则创建
      }
      ...
      try {
        ...
        writeLongToFile(ACCEPTED_EPOCH_FILENAME, acceptedEpoch); // MARK acceptedEpoch文件不存在则创建
      }
  ...
  synchronized public void startLeaderElection() {
    try {
      // MARK 设置当前vote: myid, max zxid in data tree, current epoch
  ...
  protected Observer makeObserver(FileTxnSnapLog logFactory) throws IOException {
    return new Observer(this, new ObserverZooKeeperServer(logFactory, this,
        new ZooKeeperServer.BasicDataTreeBuilder(), this.zkDb));
  }
  ...
  protected Election createElectionAlgorithm(int electionAlgorithm) {
    ...
    case 2:
      ...
      break;
    case 3:
      qcm = new QuorumCnxManager(this);           // MARK 创建QuorumCnxManager
      QuorumCnxManager.Listener listener = qcm.listener;
      if (listener != null) {
        listener.start();                         // MARK 启动QuorumCnxManager.Listener: 监听选举端口
        le = new FastLeaderElection(this, qcm);   // MARK 创建选举算法实例: 启动WorkerSender和WorkerReceiver 
      } else {
  ...
  synchronized public ZooKeeperServer getActiveServer() {
    if (leader != null) return leader.zk;
    else if (follower != null) return follower.zk;
    else if (observer != null) return observer.zk;
    return null;
  }
  ...
  public void run() {
    ...
    try {
      ...
      while (running) {
        switch (getPeerState()) {
        // MARK case 1: ServerState=LOOKING; 初始状态
        case LOOKING:
          LOG.info("LOOKING");
          ...
          // MARK 非只读
            ...
            try {
              setBCVote(null);
              // MARK (1) 开始新一轮的leader选举, (2) 设置当前的投票
              setCurrentVote(makeLEStrategy().lookForLeader());
            } catch (Exception e) {
              LOG.warn("Unexpected exception", e);
              setPeerState(ServerState.LOOKING);
        ...
        // MARK case 2: ServerState=OBSERVING
        case OBSERVING:
          try {
            LOG.info("OBSERVING");
            setObserver(makeObserver(logFactory)); // MARK 创建observer
        ...
        // MARK case 3: ServerState=FOLLOWING
        case FOLLOWING:
          try {
            LOG.info("FOLLOWING");
            setFollower(makeFollower(logFactory)); // MARK 创建follower
        ...
        // MARK case 4: ServerState=LEADING
        case LEADING:
          LOG.info("LEADING");
          try {
            setLeader(makeLeader(logFactory)); // MARK 创建leader
            leader.lead();
            setLeader(null);
          } catch (Exception e) {
            LOG.warn("Unexpected exception", e);
  ...
  /**
   * MARK 获取quorum中参与者. Observers are not contained in this view, only nodes with
      ...
      if (leader != null) {
        for (LearnerHandler fh : leader.getLearners()) {
          if (fh.getSocket() != null) {
            ...
            if (leader.isLearnerSynced(fh)) s += "*";
            l.add(s);
          }
        }
   ...
   * ensemble. See ZOOKEEPER-1732 for more info.
   */
  protected void updateElectionVote(long newEpoch) {
    ...
    if (currentVote != null) {
      setCurrentVote(
        new Vote(currentVote.getId(), currentVote.getZxid(),
          currentVote.getElectionEpoch(), newEpoch, currentVote.getState()));
    }
  }
